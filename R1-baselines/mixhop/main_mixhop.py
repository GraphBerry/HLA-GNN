...
class MixHopConv(nn.Module):
    ...
    Description
    -----------
    MixHop Graph Convolutional layer from paper `MixHop: Higher-Order Graph Convolutional 
...
def main(args, split_id=0):
    ...
    graph, features, labels, n_classes = build_graph(args.dataset)
    train_idx, val_idx, test_idx = split_dataset(args.dataset, split_id)
    ...
    scheduler = optim.lr_scheduler.StepLR(
        opt, args.step_size, gamma=args.gamma)
...
if __name__ == "__main__":
    ...
    # model params
    parser.add_argument(
        "--hid-dim", type=int, default=128, help="Hidden layer dimensionalities."
    )
    parser.add_argument(
    ...
    for i in range(10):
        acc_lists.append(main(args, i) * 100)
