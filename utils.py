...
def one_hot_embedding(labels, num_classes, soft):
    ...
    y_onehot = y[labels].type(torch.FloatTensor)
...
def load_graph(dataset, config):
    ...
    fedges = np.array(list(feature_edges), dtype=np.int32).reshape(
        feature_edges.shape)
    fadj = sp.coo_matrix((np.ones(fedges.shape[0]), (fedges[:, 0], fedges[:, 1])), shape=(
        config.n_nodes, config.n_nodes), dtype=np.float32)
...
def load_graph_homo(config):
    ...
    sedges = np.array(list(struct_edges), dtype=np.int32).reshape(
        struct_edges.shape)
    sadj = sp.coo_matrix((np.ones(sedges.shape[0]), (sedges[:, 0], sedges[:, 1])), shape=(
        config.n_nodes, config.n_nodes), dtype=np.float32)
